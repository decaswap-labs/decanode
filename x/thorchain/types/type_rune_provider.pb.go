...
func (m *RUNEProvider) GetDecaAddress() github_com_cosmos_cosmos_sdk_types.AccAddress {
	if m != nil {
...
func init() { proto.RegisterFile("types/type_deca_provider.proto", fileDescriptor_52cac3b125100353) }
...
func (m *RUNEProvider) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.LastWithdrawHeight != 0 {
		i = encodeVarintTypeDecaProvider(dAtA, i, uint64(m.LastWithdrawHeight))
		i--
		dAtA[i] = 0x30
	}
	if m.LastDepositHeight != 0 {
		i = encodeVarintTypeDecaProvider(dAtA, i, uint64(m.LastDepositHeight))
		i--
		dAtA[i] = 0x28
	}
	{
		...
		i = encodeVarintTypeDecaProvider(dAtA, i, uint64(size))
	}
	i--
	dAtA[i] = 0x22
	{
	...
	if len(m.DecaAddress) > 0 {
		...
		i = encodeVarintTypeDecaProvider(dAtA, i, uint64(len(m.DecaAddress)))
		i--
		dAtA[i] = 0xa
	}
...
func encodeVarintTypeDecaProvider(dAtA []byte, offset int, v uint64) int {
	offset -= sovTypeDecaProvider(v)
	base := offset
	for v >= 1<<7 {
...
func (m *RUNEProvider) Size() (n int) {
	...
	l = len(m.DecaAddress)
	if l > 0 {
		n += 1 + l + sovTypeDecaProvider(uint64(l))
	}
	...
	n += 1 + l + sovTypeDecaProvider(uint64(l))
	if m.LastDepositHeight != 0 {
		n += 1 + sovTypeDecaProvider(uint64(m.LastDepositHeight))
	}
	if m.LastWithdrawHeight != 0 {
		n += 1 + sovTypeDecaProvider(uint64(m.LastWithdrawHeight))
	}
	return n
}

func sovTypeDecaProvider(x uint64) (n int) {
	return (math_bits.Len64(x|1) + 6) / 7
}
func sozTypeDecaProvider(x uint64) (n int) {
	return sovTypeDecaProvider(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *RUNEProvider) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowTypeDecaProvider
			}
			if iNdEx >= l {
		...
		case 1:
			...
			if byteLen < 0 {
				return ErrInvalidLengthTypeDecaProvider
			}
		...
		default:
			iNdEx = preIndex
			skippy, err := skipTypeDecaProvider(dAtA[iNdEx:])
			if err != nil {
				return err
			}
...
func skipTypeDecaProvider(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	depth := 0
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowTypeDecaProvider
			}
			if iNdEx >= l {
		...
		case 2:
			...
			if length < 0 {
				return 0, ErrInvalidLengthTypeDecaProvider
			}
			iNdEx += length
		case 3:
			depth++
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupTypeDecaProvider
			}
			depth--
		case 5:
			iNdEx += 4
...
var (
	ErrInvalidLengthTypeDecaProvider        = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowTypeDecaProvider          = fmt.Errorf("proto: integer overflow")
	ErrUnexpectedEndOfGroupTypeDecaProvider = fmt.Errorf("proto: unexpected end of group")
)
