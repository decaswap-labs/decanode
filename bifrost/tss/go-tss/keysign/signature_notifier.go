...
func (s *SignatureNotifier) handleStream(stream network.Stream) {
	...
	if len(msg.Signatures) > 0 && msg.KeysignStatus == messages.KeysignSignature_Success {
		signatures := make([]*common.SignatureData, len(msg.Signatures))
		for i, el := range msg.Signatures {
...
func (s *SignatureNotifier) getNotifier(
	messageID string,
	messages [][]byte,
	poolPubKey string,
	...
	if err != nil {
		s.logger.Error().Err(err).Msgf("fail to create notifier for messageID %s", messageID)
		return nil, err
	}
...
func (s *SignatureNotifier) createOrUpdateNotifier(
	...
	n, err := s.getNotifier(messageID, messages, poolPubKey, signatures)
	if err != nil {
		return nil, fmt.Errorf("fail to get notifier: %w", err)
	}
	...
	if n.readyToProcess() {
		if err := n.processSignature(n.signatures); err != nil {
			return nil, err
		}
	}
	return n, nil
...
func (s *SignatureNotifier) WaitForSignature(messageID string, messages [][]byte, poolPubKey string, timeout time.Duration, sigChan chan string) ([]*tsslibcommon.SignatureData, error) {
	s.logger.Debug().Msg("waiting for signature")
	n, err := s.createOrUpdateNotifier(messageID, messages, poolPubKey, nil, timeout+time.Second)
	if err != nil {
