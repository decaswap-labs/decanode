...
func (tKeyGen *EDDSAKeyGen) GenerateNewKey(keygenReq keygen.Request) (*bcrypto.ECPoint, error) {
	...
	bufferSize := len(partiesID) * 10 // buffer size for outCh and endCh
	outCh := make(chan btss.Message, bufferSize)
	endCh := make(chan eddsakg.LocalPartySaveData, bufferSize)
...
func (tKeyGen *EDDSAKeyGen) processKeyGen(errChan chan struct{},
	...
	for {
		...
		case msg, ok := <-outCh:
			if !ok {
				return nil, errors.New("outCh closed unexpectedly")
			}
		...
		case msg, ok := <-endCh:
			if !ok {
				return nil, errors.New("endCh closed unexpectedly")
			}
