...
func (s *TssECDSAKeygenTestSuite) SetUpTest(c *C) {
	...
	for i := 0; i < s.partyNum; i++ {
		...
		c.Assert(err, IsNil)
		if i == 0 {
			comm, err := p2p.NewCommunication(&p2p.Config{Port: ports[i], RendezvousString: "asgard"}, nil, nil)
			c.Assert(err, IsNil)
			c.Assert(comm.Start(buf[:]), IsNil)
			s.comms[i] = comm
			continue
		}
		comm, err := p2p.NewCommunication(&p2p.Config{Port: ports[i], RendezvousString: "asgard", BootstrapPeers: []maddr.Multiaddr{multiAddr}}, nil, nil)
		c.Assert(err, IsNil)
		c.Assert(comm.Start(buf[:]), IsNil)
		s.comms[i] = comm
	}
...
func (s *TssECDSAKeygenTestSuite) TestGenerateNewKeyWithStop(c *C) {
	...
	for i := 0; i < s.partyNum; i++ {
		...
		go func(idx int) {
			...
			sort.Strings(testPubKeys)
			req := keygen.NewRequest(localpubKey, 10, "", tcommon.SigningAlgoSecp256k1)
			messageID, err := common.MsgToHashString([]byte(strings.Join(req.Keys, "")))
			c.Assert(err, IsNil)
			comm := s.comms[idx]
			stopChan := make(chan struct{})
