...
func main() {
	...
	url := fmt.Sprintf("%s/thorchain/nodes", getEnvOrDefault("THORNODE", "https://gateway.liquify.com/chain/thorchain_api"))
	// nolint
	...
	height, err := fetchBlockHeight(getEnvOrDefault("RPC", "https://gateway.liquify.com/chain/thorchain_rpc"))
	if err != nil {
		panic(err)
	}
	wg := sync.WaitGroup{}
